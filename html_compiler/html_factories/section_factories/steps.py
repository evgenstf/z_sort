def build_first_step(subsections, article_url):
    step_subbody_html = ''
    ...
    step_subbody_html += compile_section(subsections, article_url)
    step_subbody_html += '</div>'
    step_subbody_html += '</div>'
...
def build_middle_steps(subsections, article_url):
    step_subbody_html = ''
...
def build_last_step(subsections, article_url):
    step_subbody_html = ''
...
class StepSectionFactory:
    @staticmethod
    def build_html(subsections, article_url):
        step_body_html = ''
        if type(subsections) == str:
            import json
            subsections = json.loads(' '.join(str(subsections).split(' ')))
        step_body_html += build_first_step(subsections[0], article_url)
        if subsections[1:-1]:
            step_body_html += build_middle_steps(subsections[1:-1], article_url)
        step_body_html += build_last_step(subsections[-1], len(subsections), article_url)
